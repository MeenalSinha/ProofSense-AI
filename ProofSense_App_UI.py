...
# Configure page
st.set_page_config(
    page_title="ProofSense AI - Verifiability Analysis",
    page_icon="PS",
    layout="wide",
...
<style>
    ...
    * {
    ...
    .hero-section {
        background: linear-gradient(135deg, rgba(240, 242, 245, 0.95), rgba(230, 235, 240, 0.95));
    ...
    .metric-glass-card:hover {
        ...
        box-shadow: 0 10px 35px rgba(200, 184, 219, 0.3);
    }
...
class ProofSenseEngine:
    ...
    def generate_explanation(self, claim: str, score: float, evidence_list: List[Tuple[str, float]], 
        ...
        if score >= 70:
            ...
            explanation = f"This claim has moderate support but could benefit from additional verification (verifiability confidence: {score:.1f}/100)."
        elif score >= 30:
            explanation = f"This claim has weak support in available sources (verifiability confidence: {score:.1f}/100). Exercise caution."
        else:
    ...
    def verify_answer(self, answer: str) -> VerificationResult:
        ...
        # Defensive safeguard: cap at 10 claims to prevent UI overload
        claim_texts = claim_texts[:10]
...
def display_claim(claim: Claim, claim_num: int):
    ...
    with col3:
        if claim.warnings:
            for warning in claim.warnings:
                st.markdown(f"""
                <span class="warning-badge badge-orange">{warning}</span>
                """, unsafe_allow_html=True)
    
    # Add "Why This Matters" tooltip for high-risk claims
    if claim.risk_level == "high":
        st.warning("**Why This Matters:** High-risk claims can mislead users if acted upon. Always verify with authoritative sources before making decisions.")
    ...
    if claim.evidence:
        ...
        st.warning("No supporting evidence found in prototype evidence store (simulated RAG for demo)")
...
def main():
    ...
    <div class="hero-section">
        ...
        <p class="hero-subtitle">Verifiability Analysis for LLM Outputs</p>
    </div>
    """, unsafe_allow_html=True)
    
    st.info("**Important:** Scores represent verifiability confidence (evidence support), not factual correctness. Always verify critical information.")
    ...
    with st.sidebar:
        ...
        if domain == "health":
            st.warning("Health information should be verified with qualified medical professionals.")
...
POST /api/verify
{
  ...
  "domain": "general"
        ...
        st.caption("**Important:** ProofSense evaluates evidence support, not factual correctness.")
    ...
    with tab1:
        ...
        if 'verification_result' in st.session_state:
            ...
            for col, (label, value, _) in zip([col1, col2, col3, col4, col5], metrics):
                with col:
                    st.markdown(f"""
    ...
    with tab2:
        if 'verification_result' in st.session_state:
            ...
            if unsupported_ratio > 30:
                st.warning(f"**{unsupported_ratio:.0f}% of claims lack sufficient evidence** - Exercise caution with this content")
            elif unsupported_ratio > 0:
                st.info(f"{unsupported_ratio:.0f}% of claims lack sufficient evidence")
            else:
            ...
            # Warning summary
            st.subheader("Language Warnings Summary")
    ...
    with tab3:
        if 'verification_result' in st.session_state:
            ...
            st.markdown("### Export Trust Report")
...
- High Risk: {result.risk_distribution['high']}
            ...
            if report_format == "Text Summary":
                ...
                summary_items = [
                    ...
                    f"**Flagged Claims:** {result.flagged_claims}",
    ...
    <div class="footer">
        ...
        <div style="margin: 1.5rem 0;">
            <span class="tech-badge">Prototype RAG</span>
            <span class="tech-badge">Interactive Analysis</span>
            <span class="tech-badge">Multi-Factor Scoring</span>
            <span class="tech-badge">Production-Oriented Architecture</span>
        </div>
        ...
        <p style="opacity: 0.7; font-size: 0.95rem; margin-top: 2rem;">
            ...
            Powered by Python + Streamlit (prototype RAG) | 2025 ProofSense AI
        </p>
